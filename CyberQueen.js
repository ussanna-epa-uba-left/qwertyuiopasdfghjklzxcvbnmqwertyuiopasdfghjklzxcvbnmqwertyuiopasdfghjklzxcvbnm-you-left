...
const {WAConnection, MessageOptions, MessageType, Mimetype, Presence} = require('cyber-queen-web-api');
...
// Sql
const CyberQueenDB = config.DATABASE.define('CyberQueen', {
    info: {
...
async function CyberquennW () {
    ...
    var StrSes_Db = await CyberQueenDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await CyberQueenDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
    ...
    conn.on('open', async () => {
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/CyberQueen-alive.png"), MessageType.image, { caption: '*᳆⃞🇱🇰ᴄʏͥʙᴇͣʀͫ↯ǫᴜᴇᴇɴᬐ private ආකාරයට ක්‍රියා කරයි.*\n\n*Username:* ' + conn.user.name + '\n\n_මෙහි command උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකය වේ._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ command list එක ලබාගැනීමට .panel command බාවිතා කල හැකිය.*\n\n*ඔබේ bot private ක්‍රියාත්මක වේ. වෙනස් කිරීමට* _.var WORK_TYPE:public_ *විධානය භාවිතා කරන්න.*\n\n*᳆⃞🇱🇰ᴄʏͥʙᴇͣʀͫ↯ǫᴜᴇᴇɴᬐ භාවිතා කිරීම ගැන ස්තූතියි.*', });
                }
            }
            else {
